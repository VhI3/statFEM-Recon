function BVP = infinitePlate_statFEM_processLE(BVP, n_0)
% INFINITEPLATE_STATFEM_PROCESSLE Computes the statFEM posterior solution.
%   This function computes the posterior mean and covariance for the
%   displacements in a linear elasticity finite element model using
%   statistical assimilation.
%
% Inputs:
%   BVP  - Boundary value problem structure.
%   n_0  - Number of assimilation read.
%
% Outputs:
...
cov_ux_active = BVP.lsNIPCE.LE.cov_ux_active_pc;
cov_uy_active = BVP.lsNIPCE.LE.cov_uy_active_pc;
mean_ux_red = BVP.lsNIPCE.LE.mean_ux_active_pc;
mean_uy_red = BVP.lsNIPCE.LE.mean_uy_active_pc;
mean_u = BVP.lsNIPCE.LE.mean_u_pc;

% Hyperparameters and covariance matrices
rho_x = BVP.hyp.LE.rho(1); % rho for X dimension
rho_y = BVP.hyp.LE.rho(2); % rho for Y dimension
C_e = BVP.hyp.LE.C_e;
C_d_x = BVP.hyp.LE.C_d{1}; % Covariance for X dimension
C_d_y = BVP.hyp.LE.C_d{2}; % Covariance for Y dimension

% System matrices from FEM processing
Krr = BVP.proc.LE.Krr;
Kru = BVP.proc.LE.Kru;
Kur = BVP.proc.LE.Kur;
Kuu = BVP.proc.LE.Kuu;
...
Ur = mean_u_y(activeDOFs);
Uu = mean_u_y(prescribedDOFs);
balancedEq = zeros(GDOFs, 1);
balancedEq(activeDOFs) = Krr * Ur + Kru * Uu - Rr;
balancedEq(prescribedDOFs) = Kur * Ur + Kuu * Uu - Ru;

balancedEq_x = balancedEq(1:2:end);
balancedEq_y = balancedEq(2:2:end);
balancedEq_x_Surf = makeSurf(elementNodes, balancedEq_x);
balancedEq_y_Surf = makeSurf(elementNodes, balancedEq_y);
norm_balancedEq = norm(balancedEq);

%% Assign Results Back to BVP
BVP.statFEM.LE.nAssimilation = n_0;
BVP.statFEM.LE.mean_u_y = mean_u_y;
BVP.statFEM.LE.mean_ux_y = mean_ux_y;
BVP.statFEM.LE.mean_uy_y = mean_uy_y;
BVP.statFEM.LE.mean_ux_y_Surf = mean_ux_y_Surf;
BVP.statFEM.LE.cov_u_y = cov_u_y;
BVP.statFEM.LE.std_u_y_x_Surf = std_u_y_x_Surf;
BVP.statFEM.LE.mean_u_y_bottomNodesDofx = mean_u_y_bottomNodesDofx;
BVP.statFEM.LE.ci_cov_u_y_bottomNodesDofx = ci_cov_u_y_bottomNodesDofx;

BVP.statFEM.LE.mean_UZx_y_red = mean_UZx_y_red;
BVP.statFEM.LE.ci_UZx_y_red = ci_UZx_y_red;
BVP.statFEM.LE.mean_UZy_y_red = mean_UZy_y_red;
BVP.statFEM.LE.ci_UZy_y_red = ci_UZy_y_red;
BVP.statFEM.LE.n_0 = n_0;

BVP.statFEM.LE.balancedEq = balancedEq;
BVP.statFEM.LE.balancedEq_x = balancedEq_x;
BVP.statFEM.LE.balancedEq_y = balancedEq_y;
BVP.statFEM.LE.balancedEq_x_Surf = balancedEq_x_Surf;
BVP.statFEM.LE.balancedEq_y_Surf = balancedEq_y_Surf;
BVP.statFEM.LE.norm_balancedEq = norm_balancedEq;
